# This file is adapted from https://github.com/ise-uiuc/magicoder/blob/main/src/magicoder/llm_wrapper.py
...
class TokenizationContext:
    ...
    bos_token: str
    eos_token: str
    special_token_map: dict[int, int] | None = None
    ...
    def from_model_key(
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, use_fast=use_fast, trust_remote_code=True)
    ...
    def encode(self, config: EncodingConfig, text_list: list[str]) -> list[list[int]]:
        ...
        if self.special_token_map:
            ...
            input_ids = [
                [
                    self.special_token_map[id] if id in self.special_token_map else id
                    for id in input_id
                ]
    ...
    # def set_special_tokens(self, special_tokens: list[str]):
    #     self.tokenizer.add_tokens(special_tokens)

    def set_special_tokens(self, special_tokens: list[str], start_id=None):
        self.special_token_map = {}
        vocab_size = len(self.tokenizer.vocab)
        if start_id is None:
            start_id = vocab_size - len(special_tokens)
        for i, token in enumerate(special_tokens):
            self.special_token_map[vocab_size + i] = start_id + i
        self.tokenizer.add_tokens(special_tokens)
...
class SupportedModelKeys(Enum):
    ...
    STARCODER_1B = "bigcode/starcoderbase-1b"
    ...
    def starcoder_based_models() -> list[str]:
        return [
            SupportedModelKeys.STARCODER_1B.value,
